...
void SyncPositionsWithBackend()
{
   ...
   for(int j = tracked_count-1; j >= 0; j--)
   {
      ...
      if(!found)
      {
         ...
         if(GlobalVariableCheck(max_price_key)) mfe = GlobalVariableGet(max_price_key);
         if(GlobalVariableCheck(min_price_key)) mae = GlobalVariableGet(min_price_key);
   ...
   if(pending_bos_direction != 0)
   {
      ...
      else
      {
         ...
         if(pending_bos_confirmations >= InpBOSConfirmBars)
         {
            ...
            if(trend > 0) choch_bear = false;
            else choch_bull = false;
            if(log_event)
...
void CheckBEAndTrailing()
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         string gk = GetAnalyticsKey("MAX_PRICE", analytics_identity);
      ...
      {
         string gk = GetAnalyticsKey("MIN_PRICE", analytics_identity);
...
bool ValidateInputParameters()
{
   ...
   if(InpBOSConfirmBars < 1 || InpBOSMaxPendingBars < InpBOSConfirmBars
      || InpFVGMaxAgeBars < 1 || InpOBMaxAgeBars < 1)
   {
      Print("ATS EA ERROR: BOS confirmation/expiry and FVG/OB maximum ages are invalid.");
      return false;
   }
   ...
   if(InpBEPips <= 0 || InpTrailLevel1Pips <= 0
